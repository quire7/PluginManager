...
NDBufferPool::NDBufferPool()
{
	...
	for (int i = 0; i < maxTinyBuffers; ++i) {
		tinyPool[i] = (unsigned char*)tinyHeap + (tinyBufferSize * i);
	}
...
//	}
	...
	*(unsigned int*)ptr = (unsigned int)bytes;
...
void NDBufferPool::free( void* ptr )
{
	...
	assert(NDStaticFunctionGlobal::isValidPointer(ptr));
	...
	unsigned int bytes = USERSIZE_FROM_USERPTR(ptr);
...
std::string NDBufferPool::performance() const
{
	if (totalMallocs > 0) {
		...
		char szBuf[128] = {0};
		SNPRINTF( szBuf, 127, "malloc performance: %5.1f%% <= %db, %5.1f%% <= %db, "
							   "%5.1f%% <= %db, %5.1f%% > %db",
...
{
	char szBuf[128] = {0};
	SNPRINTF( szBuf, sizeof(szBuf) - 1, "preallocated shared buffers: %5d/%d x %db", (maxTinyBuffers - tinyPoolSize), maxTinyBuffers, tinyBufferSize );
	return string(szBuf);
}
...
{
	return (ptr >= tinyHeap) && (ptr < (unsigned char*)tinyHeap + maxTinyBuffers * tinyBufferSize);
}
...
void NDBufferPool::lock()
{
	m_lock->lock();
}

void NDBufferPool::unlock()
{
	m_lock->unlock();
}